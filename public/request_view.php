...
$chatAllowedRoles = ['client', 'operator', 'admin'];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['send_message']) && in_array($user['role'], $chatAllowedRoles, true)) {
        $message = trim($_POST['message'] ?? '');

        if ($message !== '') {
            $msgStmt = db()->prepare(
                'INSERT INTO request_messages (request_id, sender_id, message) VALUES (:request_id, :sender_id, :message)'
            );
            $msgStmt->execute([
            ...
            flash('error', 'Сообщение не должно быть пустым.');
        }
...
$messageStmt = db()->prepare(
    'SELECT rm.message, rm.created_at, u.name AS sender_name, u.role AS sender_role
     FROM request_messages rm
     JOIN users u ON u.id = rm.sender_id
     WHERE rm.request_id = :id
     ORDER BY rm.created_at ASC'
);
$messageStmt->execute(['id' => $requestId]);
...
<div class="row g-3 mt-1">
    <div class="col-lg-6">
        ...
        <?php if (in_array($user['role'], $chatAllowedRoles, true)): ?>
            <div class="card shadow-sm mt-3">
                <div class="card-body">
                    <h2 class="h5">Чат клиента и оператора</h2>
                    <div class="border rounded p-3 mb-3" style="max-height: 260px; overflow-y: auto; background: #fff;">
                        <?php if ($messages === []): ?>
                            <p class="text-muted mb-0">Пока сообщений нет.</p>
                        <?php else: ?>
                            <?php foreach ($messages as $message): ?>
                                <div class="mb-2">
                                    <strong><?= e($message['sender_name']) ?></strong>
                                    <span class="badge bg-info text-dark"><?= e($message['sender_role']) ?></span><br>
                                    <span><?= nl2br(e($message['message'])) ?></span><br>
                                    <small class="text-muted"><?= e($message['created_at']) ?></small>
                                </div>
                            <?php endforeach; ?>
                        <?php endif; ?>
                    </div>
                    <form method="post">
                        ...
                        <div class="mb-2">
                            <textarea name="message" class="form-control" rows="3" placeholder="Введите сообщение..." required></textarea>
                        </div>
